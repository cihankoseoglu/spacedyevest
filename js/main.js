...
w=480,h=260,svg=d3.select("#chart").append("svg").attr({width:w,height:h}).append("g").attr("transform","translate(20,10)");
...
function rise(cell) {
	...
	cell.attr({x:10,y:10+d.row*20,height:0,width:0}).style("fill",liveC).transition().delay(1500).duration(1500).ease("elastic").attr({x:1,y:1+d.row*20,height:18,width:18})
}
